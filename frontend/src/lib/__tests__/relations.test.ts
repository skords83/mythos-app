...
describe('isValidEntityType', () => {
  it('accepts CHARACTER, PLACE, ITEM, FACTION, SCENE, EVENT and LORE_ENTRY', () => {
    ...
    expect(isValidEntityType('LORE_ENTRY')).toBe(true)
  })
...
describe('resolveEntity', () => {
  it('queries the character table for CHARACTER and scopes by familyId', async () => {
    ...
    const prisma = { character: { findFirst }, place: { findFirst: jest.fn() }, item: { findFirst: jest.fn() }, faction: { findFirst: jest.fn() }, scene: { findFirst: jest.fn() }, timelineEvent: { findFirst: jest.fn() }, loreEntry: { findFirst: jest.fn() } }
  ...
  it('queries the place table for PLACE', async () => {
    ...
    const prisma = { character: { findFirst: jest.fn() }, place: { findFirst }, item: { findFirst: jest.fn() }, faction: { findFirst: jest.fn() }, scene: { findFirst: jest.fn() }, timelineEvent: { findFirst: jest.fn() }, loreEntry: { findFirst: jest.fn() } }
  ...
  it('queries the faction table for FACTION', async () => {
    ...
    const prisma = { character: { findFirst: jest.fn() }, place: { findFirst: jest.fn() }, item: { findFirst: jest.fn() }, faction: { findFirst }, scene: { findFirst: jest.fn() }, timelineEvent: { findFirst: jest.fn() }, loreEntry: { findFirst: jest.fn() } }
  ...
  it('queries the scene table for SCENE', async () => {
    ...
    const prisma = { character: { findFirst: jest.fn() }, place: { findFirst: jest.fn() }, item: { findFirst: jest.fn() }, faction: { findFirst: jest.fn() }, scene: { findFirst }, timelineEvent: { findFirst: jest.fn() }, loreEntry: { findFirst: jest.fn() } }
  ...
  it('queries the timelineEvent table for EVENT', async () => {
    ...
    const prisma = { character: { findFirst: jest.fn() }, place: { findFirst: jest.fn() }, item: { findFirst: jest.fn() }, faction: { findFirst: jest.fn() }, scene: { findFirst: jest.fn() }, timelineEvent: { findFirst }, loreEntry: { findFirst: jest.fn() } }
  ...
  it('queries the loreEntry table for LORE_ENTRY', async () => {
    const findFirst = jest.fn().mockResolvedValue({ id: 'l1', familyId: 'fam-1', authorId: 'user-1', visibility: 'FAMILY' })
    const prisma = { character: { findFirst: jest.fn() }, place: { findFirst: jest.fn() }, item: { findFirst: jest.fn() }, faction: { findFirst: jest.fn() }, scene: { findFirst: jest.fn() }, timelineEvent: { findFirst: jest.fn() }, loreEntry: { findFirst } }

    const result = await resolveEntity(prisma, 'LORE_ENTRY', 'l1', 'fam-1')

    expect(findFirst).toHaveBeenCalledWith(
      expect.objectContaining({ where: { id: 'l1', familyId: 'fam-1' } })
    )
    expect(result).toEqual({ id: 'l1', familyId: 'fam-1', authorId: 'user-1', visibility: 'FAMILY' })
  })
})
