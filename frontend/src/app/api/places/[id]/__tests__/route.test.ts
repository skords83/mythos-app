...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    place: { findFirst: jest.fn(), findUnique: jest.fn(), update: jest.fn(), findMany: jest.fn(), updateMany: jest.fn(), delete: jest.fn() },
...
const mockedPrisma = prisma as unknown as {
  place: { findFirst: jest.Mock; findUnique: jest.Mock; update: jest.Mock; findMany: jest.Mock; updateMany: jest.Mock; delete: jest.Mock }
...
describe('DELETE /api/places/[id]', () => {
  it('deletes relations referencing the place before deleting it (Z5 cascade)', async () => {
    mockedPrisma.place.findFirst.mockResolvedValueOnce({ id: 'place-1', authorId: 'user-1' })
    mockedPrisma.relation.deleteMany.mockResolvedValue({ count: 2 })
    mockedPrisma.place.delete.mockResolvedValue({ id: 'place-1' })

    const request = authedRequest('http://localhost/api/places/place-1', { method: 'DELETE' })
    const response = await DELETE(request, { params: { id: 'place-1' } })

    expect(response.status).toBe(200)
    expect(mockedPrisma.relation.deleteMany).toHaveBeenCalledWith({
      where: { OR: [{ sourceType: 'PLACE', sourceId: 'place-1' }, { targetType: 'PLACE', targetId: 'place-1' }] },
    })
    expect(mockedPrisma.place.delete).toHaveBeenCalledWith({ where: { id: 'place-1' } })
  })

  it('returns 403 without touching relations when the caller is not the author', async () => {
    mockedPrisma.place.findFirst.mockResolvedValueOnce({ id: 'place-1', authorId: 'other-user' })
    ...
    expect(mockedPrisma.place.delete).not.toHaveBeenCalled()
  })
})
