...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    ...
    idea: { findMany: jest.fn(), create: jest.fn() },
  },
}))
...
const mockedPrisma = prisma as unknown as {
  ...
  idea: { findMany: jest.Mock; create: jest.Mock }
}
...
describe('GET /api/ideas', () => {
  ...
  it('scopes a project-bound query by family, not just the caller', async () => {
    ...
    const response = await GET(authedRequest('http://localhost/api/ideas?projectId=proj-1'))
...
describe('POST /api/ideas', () => {
  it('creates a family-board idea without a projectId', async () => {
    mockedPrisma.idea.create.mockResolvedValue({ id: 'idea-1' })
    const request = authedRequest('http://localhost/api/ideas', {
      method: 'POST',
      body: JSON.stringify({ title: 'Ein Zwilling taucht auf', visibility: 'FAMILY' }),
    })
    const response = await POST(request)
    expect(response.status).toBe(201)
    expect(mockedPrisma.idea.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ familyId: 'fam-1', authorId: 'user-1', visibility: 'FAMILY', projectId: null, tags: [] }),
      })
    )
  })

  it('passes tags through as-is', async () => {
    mockedPrisma.idea.create.mockResolvedValue({ id: 'idea-1' })
    const request = authedRequest('http://localhost/api/ideas', {
      method: 'POST',
      body: JSON.stringify({ title: 'Plot-Twist', tags: ['Plot', 'Wendepunkt'] }),
    })
    const response = await POST(request)
    expect(response.status).toBe(201)
    expect(mockedPrisma.idea.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ tags: ['Plot', 'Wendepunkt'] }) })
    )
  })
  ...
  it('rejects an invalid visibility value', async () => {
    const request = authedRequest('http://localhost/api/ideas', {
      method: 'POST',
      body: JSON.stringify({ title: 'Ein Zwilling taucht auf', visibility: 'PUBLIC' }),
    })
    const response = await POST(request)
