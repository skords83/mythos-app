...
jest.mock('@/lib/prisma', () => ({
  prisma: {
    ...
    item: { findMany: jest.fn(), create: jest.fn() },
  },
}))
...
const mockedPrisma = prisma as unknown as {
  ...
  item: { findMany: jest.Mock; create: jest.Mock }
}
...
describe('GET /api/items', () => {
  ...
  it('scopes a project-bound query by family, not just the caller', async () => {
    ...
    const response = await GET(authedRequest('http://localhost/api/items?projectId=proj-1'))
  ...
  it('returns the family-wide library when no projectId is given', async () => {
    mockedPrisma.item.findMany.mockResolvedValue([])
    const response = await GET(authedRequest('http://localhost/api/items'))
    expect(response.status).toBe(200)
...
describe('POST /api/items', () => {
  it('creates a family-library item without a projectId', async () => {
    mockedPrisma.item.create.mockResolvedValue({ id: 'item-1' })
    const request = authedRequest('http://localhost/api/items', {
      method: 'POST',
      body: JSON.stringify({ name: 'Schwert der Dämmerung', visibility: 'FAMILY' }),
    })
  ...
  it('rejects an invalid visibility value', async () => {
    const request = authedRequest('http://localhost/api/items', {
      method: 'POST',
      body: JSON.stringify({ name: 'Schwert der Dämmerung', visibility: 'PUBLIC' }),
    })
    const response = await POST(request)
