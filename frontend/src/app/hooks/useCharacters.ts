...
export function useCharacters({ selectedProject, showError, requestConfirm, onConfirmed }: UseCharactersArgs) {
  ...
  const addCharacter = async (name: string, appearance: string, personality: string, backstory: string, motivation: string, visibility: 'PRIVATE' | 'FAMILY') => {
    if (!selectedProject) return
    try {
      const response = await fetch('/api/characters', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, appearance, personality, backstory, motivation, visibility, projectId: selectedProject.id })
      })
      if (!response.ok) {
