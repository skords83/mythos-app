...
interface Relation {
  id: string
  sourceType: 'SCENE' | 'CHARACTER' | 'PLACE' | 'ITEM'
  sourceId: string
  targetType: 'SCENE' | 'CHARACTER' | 'PLACE' | 'ITEM'
  targetId: string
  relationType: string
  label: string | null
  counterpart: { type: 'SCENE' | 'CHARACTER' | 'PLACE' | 'ITEM'; id: string }
}
...
interface SceneEntityTagsProps {
  scene: Scene
  entityType: 'CHARACTER' | 'PLACE' | 'ITEM'
  label: string
  entities: EntityOption[]
}

export function SceneEntityTags({ scene, entityType, label, entities }: SceneEntityTagsProps) {
  ...
  useEffect(() => {
    let cancelled = false
    setError('')
    fetch(`/api/relations?entityType=SCENE&entityId=${scene.id}`)
